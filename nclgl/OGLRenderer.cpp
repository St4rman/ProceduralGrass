...
OGLRenderer::OGLRenderer(Window &window)	{
	...
	int attribs[] = {
		...
		,WGL_CONTEXT_PROFILE_MASK_ARB,GL_CONTEXT_COMPATIBILITY_PROFILE_BIT,		//We want everything OpenGL 3.2 provides...
...
/*
* sets shader light uniforms lightPos, lightColor, lightRadius
*/
void OGLRenderer::SetShaderLight(const Light& light) {
	...
	glUniform4fv(glGetUniformLocation(currentShader->GetProgram(), "lightColor"), 1, (float*)&light.GetColor());
...
/*This sets a texture to repeat, note: right now it doesn't bind your texture.*/
void OGLRenderer::SetTextureRepeating(GLuint target, bool state) {

	glBindTexture(GL_TEXTURE_2D, target);

	/*glTextureParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, state ? GL_REPEAT : GL_CLAMP);
	glTextureParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, state ? GL_REPEAT : GL_CLAMP);*/
	if (state) {
...
void OGLRenderer::EnableDepthTest(bool flag) {
	if (flag) {
...
void OGLRenderer::EnableCullFace(bool flag) {
	if (flag) {
